...
export default function App(){
  ...
  const Browse=()=>{
    ...
    return<>
      ...
      {!bp.mobile&&<div style={{padding:"0 0 8px",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontSize:13,color:t.mut}}>📍 Within 20km of {chosenSuburb?.name||"Perth"}</div><button onClick={()=>{setChosenSuburb(null);setAddressSearch("");}} style={{...s.btnS(false),fontSize:12}}>Change location</button></div>}
  ...
  const SellerPage=({x})=>{const[mc,setMc]=useState("All");const cats=["All",...new Set(x.menu.map(m=>m.category))];const items=mc==="All"?x.menu:x.menu.filter(m=>m.category===mc);
    return<><div style={s.hdr}><button style={s.bck} onClick={back}><I d={ic.back}/></button><span style={s.hdrT}>{x.name}</span></div>
  ...
  const Sell=()=>{
    ...
    if(editingStore){const sf=storeForm;return<><div style={s.hdr}><button style={s.bck} onClick={()=>setEditingStore(false)}><I d={ic.back}/></button><span style={s.hdrT}>Customize Kitchen</span></div><div style={{...s.sec,...formW}}>
    ...
    return<><div style={s.hdr}><span style={s.hdrT}>Your Kitchen</span></div><div style={{...s.sec,...formW}}>
      <div style={{...s.card,padding:16,marginBottom:12}}><div style={{display:"flex",gap:12,alignItems:"center"}}><div style={{width:56,height:56,borderRadius:16,background:t.priL,display:"flex",alignItems:"center",justifyContent:"center",fontSize:28,flexShrink:0,overflow:"hidden"}}>{profile.shop_image_url?<img src={profile.shop_image_url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:(profile.avatar_emoji||"🍰")}</div><div style={{flex:1}}><div style={{fontWeight:700,fontSize:16}}>{profile.name}</div><div style={{fontSize:12,color:t.mut}}>{profile.handle} · {profile.suburb}, {profile.state}</div></div><button onClick={()=>{setStoreForm({name:profile.name,bio:profile.bio||"",suburb:profile.suburb,state:profile.state,avatar_emoji:profile.avatar_emoji||"🍰",delivery:profile.delivery,pickup:profile.pickup,shop_image_url:profile.shop_image_url||""});setPhotoPreview(null);setEditingStore(true);}} style={{...s.btnS(false),display:"flex",alignItems:"center",gap:4}}><I d={ic.edit} s={14}/> Edit</button></div>{profile.bio&&<p style={{fontSize:13,color:t.mut,margin:"10px 0 0",lineHeight:1.5}}>{profile.bio}</p>}<div style={{display:"flex",gap:5,marginTop:8}}>{profile.pickup&&<span style={s.badge(t.okBg,"#166534")}>Pickup</span>}{profile.delivery&&<span style={s.badge("#dbeafe","#1e40af")}>Delivery</span>}{!profile.verified&&<span style={s.badge("#fef3c7","#92400e")}>⏳ Pending</span>}</div></div>
