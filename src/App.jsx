...
export default function App(){
  ...
  const Sell=()=>{
    ...
    if(editingStore){const sf=storeForm;return<><div style={s.hdr}><button style={s.bck} onClick={()=>setEditingStore(false)}><I d={ic.back}/></button><span style={s.hdrT}>Customize Kitchen</span></div><div style={{...s.sec,...formW}}>
      ...
      <div style={{marginBottom:14}}><label style={{fontSize:13,fontWeight:600,display:"block",marginBottom:6}}>Cuisine style</label><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{CUISINES.map(c=><button key={c.name} onClick={()=>setStoreForm(p=>({...p,cuisine:c.name}))} style={{padding:"6px 12px",borderRadius:20,fontSize:13,border:sf.cuisine===c.name?`2px solid ${t.pri}`:`1.5px solid ${t.bdr}`,background:sf.cuisine===c.name?t.priL:t.card,cursor:"pointer",display:"flex",alignItems:"center",gap:5}}><span style={{fontSize:16}}>{c.flag}</span> {c.name}</button>)}</div></div>
