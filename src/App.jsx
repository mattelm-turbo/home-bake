...
const mkCss=bp=>{const mob=bp.mobile,px=mob?16:bp.tablet?24:32;return{
  page:{fontFamily:"'Inter',-apple-system,sans-serif",background:t.bg,minHeight:"100vh",color:t.txt,paddingBottom:mob?"calc(92px + env(safe-area-inset-bottom, 0px))":24},
...
export default function App(){
  ...
  async function loadSellers(){
    ...
    const{data:revData}=sellerIds.length?await supabase.from("reviews").select("*").in("seller_id",sellerIds):{data:[]};
    const revs=revData||[];
    setSellers(sellers.map(p=>{
      const pRevs=revs.filter(r=>r.seller_id===p.id);
  ...
  async function addMenuItem(item){
    ...
    const{error}=await supabase.from("menu_items").insert({seller_id:session.user.id,name:item.name,category:item.cat,price:parseFloat(item.price),description:item.desc,emoji:item.emoji,allergens:item.allergens,image_url:item.image_url||null});if(error){showToast("Error adding");return false;}await loadProfile(session.user.id);await loadSellers();return true;}
  ...
  const Sell=()=>{
    ...
    const[mf,setMf]=useState({name:"",cat:"Cakes",price:"",desc:"",allergens:[],emoji:"🍰",image_url:null});
    ...
    return<>{mobileHeader}<div style={s.hdr}><span style={s.hdrT}>Your Kitchen</span></div><div style={{...s.sec,...formW}}>
      ...
      {myMenu.map(item=><div key={item.id} onClick={()=>item.image_url&&openLightbox([{image_url:item.image_url,caption:item.name}],0)} style={{...s.card,padding:12,marginBottom:8,cursor:item.image_url?"pointer":"default"}}><div style={{display:"flex",gap:10,alignItems:"center"}}><div style={{width:44,height:44,borderRadius:10,background:"#fef3c7",display:"flex",alignItems:"center",justifyContent:"center",fontSize:22,flexShrink:0,overflow:"hidden"}}>{item.image_url?<img src={item.image_url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:item.emoji}</div><div style={{flex:1}}><div style={{fontWeight:600,fontSize:13}}>{item.name}</div><div style={{fontSize:11,color:t.mut}}>{item.category} · {money(item.price)}</div></div><button onClick={async(e)=>{e.stopPropagation();await supabase.from("menu_items").update({active:false}).eq("id",item.id);const{data}=await supabase.from("menu_items").select("*").eq("seller_id",session.user.id).eq("active",true);setMyMenu(data||[]);await loadSellers();showToast("Item removed");}} style={{background:"none",border:"none",cursor:"pointer",color:t.no,padding:4}}><I d={ic.trash} s={16}/></button></div></div>)}
      ...
      {myGallery.length>0&&<div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:16}}>
        {myGallery.map((img,i)=><div key={img.id} onClick={()=>openLightbox(myGallery,i)} style={{position:"relative",paddingBottom:"100%",borderRadius:12,overflow:"hidden",background:t.bg,cursor:"pointer"}}>
          <img src={img.image_url} alt={img.caption||""} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover"}}/>
          ...
          <button onClick={e=>{e.stopPropagation();deleteGalleryImage(img);}} aria-label="Delete photo" style={{position:"absolute",top:6,right:6,width:24,height:24,borderRadius:12,background:"rgba(0,0,0,0.6)",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><I d={ic.x} s={12} c="#fff"/></button>
        </div>)}
      </div>}
    </div></>;
  };
  ...
  const MenuItemForm=({mf,setMf,s:_s})=>{
    ...
    const handleItemPhoto=async(e)=>{
      const file=e.target.files?.[0];if(!file)return;
      if(file.size>5*1024*1024){showToast("Image must be under 5MB");return;}
      setUploadingItemPhoto(true);
      const ext=file.name.split(".").pop();
      const path=`${session.user.id}/${Date.now()}.${ext}`;
      let{error}=await supabase.storage.from("menu-images").upload(path,file,{upsert:true});
      if(error){await supabase.storage.createBucket("menu-images",{public:true});const retry=await supabase.storage.from("menu-images").upload(path,file,{upsert:true});error=retry.error;}
      if(error){showToast("Upload failed");setUploadingItemPhoto(false);return;}
      const{data:urlData}=supabase.storage.from("menu-images").getPublicUrl(path);
      if(urlData?.publicUrl)setMf(p=>({...p,image_url:urlData.publicUrl}));
    ...
    <div style={{marginBottom:12}}><label style={{fontSize:13,fontWeight:600,display:"block",marginBottom:4}}>Photo <span style={{fontWeight:400,color:t.mut}}>(optional)</span></label>
      {mf.image_url
        ? <div style={{position:"relative",height:140,borderRadius:t.rs,overflow:"hidden",background:t.bg}}>
            <img src={mf.image_url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
            <button onClick={()=>setMf(p=>({...p,image_url:null}))} aria-label="Remove photo" style={{position:"absolute",top:8,right:8,width:28,height:28,borderRadius:14,background:"rgba(0,0,0,0.6)",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}><I d={ic.x} s={14} c="#fff"/></button>
          </div>
        : <div onClick={()=>photoRef.current?.click()} style={{height:80,border:`2px dashed ${t.bdr}`,borderRadius:t.rs,display:"flex",alignItems:"center",justifyContent:"center",color:t.mut,cursor:"pointer",gap:6}}>{uploadingItemPhoto?<span style={{fontSize:13,color:t.pri,fontWeight:600}}>Uploading...</span>:<><I d={ic.cam} s={18}/><span style={{fontSize:13}}>Tap to upload</span></>}</div>}
      <input ref={photoRef} type="file" accept="image/*" style={{display:"none"}} onChange={handleItemPhoto}/>
      <div style={{fontSize:11,color:t.mut,marginTop:4}}>No photo? We'll show your chosen emoji instead.</div>
    </div>
