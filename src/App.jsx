...
const SIGNUPS_ENABLED = true; // signups are open!
...
const mkCss = (bp) => {
  const mob = bp.mobile; const px = mob ? 16 : bp.tablet ? 24 : 32;
  return {
...
export default function App() {
  ...
  async function loadSellers() {
    ...
    if (data) {
      const withMenu = data.filter(p => p.menu_items && p.menu_items.length > 0);
      setSellers(withMenu.map(p => ({
        ...p,
        menu: p.menu_items.filter(m => m.active),
        dist: Math.round(Math.random() * 15 * 10) / 10 + 0.5, // placeholder distance
  ...
  async function saveProfile(data) {
    ...
    const { error } = await supabase.from("profiles").upsert({
      id: userId, name: data.name, handle, suburb: data.suburb,
      state: data.state, bio: data.bio, avatar_emoji: data.avatar_emoji || "🍰",
      delivery: data.delivery, pickup: data.pickup,
    });
  ...
  async function addMenuItem(item) {
    const { error } = await supabase.from("menu_items").insert({
      seller_id: session.user.id, name: item.name, category: item.cat,
      price: parseFloat(item.price), description: item.desc,
      emoji: item.emoji, allergens: item.allergens,
    });
  ...
  async function loadMyMenu() {
    ...
    const { data } = await supabase.from("menu_items").select("*").eq("seller_id", session.user.id).eq("active", true);
    return data || [];
  }

  async function placeOrder(items, method, addr) {
    const grouped = {};
    items.forEach(({ seller, item, qty }) => {
      if (!grouped[seller.id]) grouped[seller.id] = { seller, items: [] };
      grouped[seller.id].items.push({ item, qty });
    });
    for (const g of Object.values(grouped)) {
      ...
      const { data: ord, error } = await supabase.from("orders").insert({
        buyer_id: session.user.id, seller_id: g.seller.id,
        method, delivery_address: addr, total, notes: "",
      }).select().single();
      if (error || !ord) { showToast("Error placing order"); return null; }
      const orderItems = g.items.map(i => ({
        order_id: ord.id, menu_item_id: i.item.id,
        item_name: i.item.name, quantity: i.qty, unit_price: i.item.price,
      }));
  ...
  const handleLogout = async () => {
    ...
    setProfile(null); setCart([]); setOrder(null); setTab("browse"); setView(null);
  };
  ...
  const addCart = (seller, item) => {
    setCart(p => { const i = p.findIndex(c => c.item.id === item.id); if (i > -1) { const n = [...p]; n[i] = { ...n[i], qty: n[i].qty + 1 }; return n; } return [...p, { seller, item, qty: 1 }]; });
  ...
  const Browse = () => {
    ...
    return <>
      ...
      <div style={{ ...s.sec, marginTop: 8 }}>
        {sellers.length === 0 && !q && <div style={{ textAlign: "center", padding: 40, color: t.mut }}><div style={{ fontSize: 44, marginBottom: 8 }}>🍰</div><div style={{ fontWeight: 600 }}>No bakers yet</div><div style={{ fontSize: 13, marginTop: 4 }}>Be the first! Go to the Sell tab to list your items.</div></div>}
        <div style={s.grid}>
          {list.map(x => (
              ...
              <div style={{ padding: 16 }}>
                <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 12, color: t.mut, marginTop: 1 }}>{x.suburb} · {x.dist}km</div>
                    <div style={{ marginTop: 3 }}><Stars r={x.rating} /> <span style={{ fontSize: 11, color: t.lit }}>({x.reviews})</span></div>
                  </div>
                </div>
                <div style={{ display: "flex", gap: 5, marginTop: 10, flexWrap: "wrap" }}>
                  {x.pickup && <span style={s.badge(t.okBg, "#166534")}>Pickup</span>}
                  {x.delivery && <span style={s.badge("#dbeafe", "#1e40af")}>Delivery</span>}
                  {x.verified && <span style={s.badge("#f0fdf4", "#166534")}>✓ Notified</span>}
                </div>
                <div style={{ display: "flex", gap: 4, marginTop: 8 }}>
                  {x.menu.slice(0, 3).map(m => <div key={m.id} style={{ width: 40, height: 40, borderRadius: 8, background: "#fef3c7", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>{m.emoji}</div>)}
                  {x.menu.length > 3 && <div style={{ width: 40, height: 40, borderRadius: 8, background: t.bg, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, color: t.mut, fontWeight: 600 }}>+{x.menu.length - 3}</div>}
                </div>
              </div>
            </div>
          ))}
        </div>
        {list.length === 0 && q && <div style={{ textAlign: "center", padding: 40, color: t.mut }}>No bakers found</div>}
      </div>
    </>;
  };
      ...
      <div style={s.sec}>
        <div style={{ ...s.card, padding: bp.mobile ? 18 : 24, marginBottom: 16 }}>
          <div style={{ display: "flex", gap: 14, alignItems: "center", marginBottom: 10 }}>
            ...
            <div><div style={{ fontWeight: 700, fontSize: bp.mobile ? 17 : 20 }}>{x.name}</div><div style={{ fontSize: 13, color: t.mut }}>{x.suburb}, {x.state}</div><Stars r={x.rating} /> <span style={{ fontSize: 11, color: t.lit }}>({x.reviews} reviews)</span></div>
          </div>
  ...
  const Cart = () => {
    ...
    if (order) return <>
      <div style={s.hdr}><span style={s.hdrT}>Confirmed!</span></div>
      <div style={{ ...s.sec, maxWidth: 500, margin: "0 auto", textAlign: "center", padding: "40px 20px" }}>
        <div style={{ fontSize: 56, marginBottom: 12 }}>🎉</div><div style={{ fontSize: 18, fontWeight: 700, marginBottom: 6 }}>Order Placed</div>
        <div style={{ color: t.mut, marginBottom: 20 }}>{order.method === "pickup" ? "Ready for pickup" : "On its way"}</div>
        <button style={s.btn(true)} onClick={() => { setOrder(null); setTab("browse"); }}>Back to Browsing</button>
      </div>
    </>;
    return <>
      <div style={s.hdr}><span style={s.hdrT}>Your Order</span></div>
      <div style={{ maxWidth: 600, margin: "0 auto" }}>
        {cart.length === 0 ? <div style={{ textAlign: "center", padding: "50px 20px", color: t.mut }}><div style={{ fontSize: 44, marginBottom: 10 }}>🛒</div><div style={{ fontWeight: 600 }}>Nothing here yet</div></div> : <div style={s.sec}>
          {cart.map(({ seller: sl, item, qty }) => <div key={item.id} style={{ ...s.card, padding: 12, marginBottom: 8 }}><div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            ...
            <div style={{ flex: 1 }}><div style={{ fontWeight: 600, fontSize: 13 }}>{item.name}</div><div style={{ fontSize: 11, color: t.mut }}>{sl.name} · {sl.suburb}</div></div>
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}><button onClick={() => qty === 1 ? rmCart(item.id) : updQty(item.id, -1)} style={{ ...s.btnS(false), padding: "2px 8px", fontSize: 14 }}>{qty === 1 ? "✕" : "−"}</button><span style={{ fontWeight: 600, minWidth: 16, textAlign: "center" }}>{qty}</span><button onClick={() => updQty(item.id, 1)} style={{ ...s.btnS(false), padding: "2px 8px", fontSize: 14 }}>+</button></div>
          ...
          <button style={{ ...s.btn(true), marginTop: 14 }} onClick={async () => {
            if (method === "delivery" && !addr.trim()) return;
            const ok = await placeOrder(cart, method, addr);
            if (ok) { setOrder({ method, total: cartT + fee }); setCart([]); }
          }}>Place Order · ${(cartT + fee).toFixed(2)}</button>
        </div>}
      </div>
    </>;
  };

  // ─── Sell (with Supabase persistence) ─────────────────────────────────────
  const Sell = () => {
    ...
    if (!profile && onboardStep === 0) return <>
      <div style={s.hdr}><span style={s.hdrT}>Start Baking & Earning</span></div>
      <div style={{ ...s.sec, ...formW, textAlign: "center", padding: "20px 0" }}>
        <div style={{ fontSize: 56, marginBottom: 12 }}>👩‍🍳</div><div style={{ fontSize: 20, fontWeight: 800, marginBottom: 6 }}>Share what you bake</div>
        <div style={{ color: t.mut, fontSize: 14, lineHeight: 1.6, marginBottom: 24 }}>Sell your homemade cakes, biscuits, preserves & sweets to people nearby.</div>
        <div style={{ display: "grid", gridTemplateColumns: bp.mobile ? "1fr" : "1fr 1fr 1fr", gap: 10, marginBottom: 20, textAlign: "left" }}>
          {[{ icon: ic.zap, title: "2 min setup", sub: "Name, suburb & first item" }, { icon: ic.shield, title: "Low-risk only", sub: "Shelf-stable cakes, biscuits, jams" }, { icon: ic.loc, title: "Local buyers", sub: "GPS-based discovery" }].map((x, i) => <div key={i} style={{ padding: 14, background: t.card, borderRadius: t.r, boxShadow: t.sh }}><div style={{ marginBottom: 6 }}><I d={x.icon} s={20} c={t.pri} /></div><div style={{ fontWeight: 700, fontSize: 13, marginBottom: 2 }}>{x.title}</div><div style={{ fontSize: 11, color: t.mut, lineHeight: 1.4 }}>{x.sub}</div></div>)}
        </div>
        <button style={s.btn(true)} onClick={() => setOnboardStep(1)}>Get Started — It's Free</button>
      </div>
    </>;

    if (!profile && onboardStep === 1) return <>
      <div style={s.hdr}><button style={s.bck} onClick={() => setOnboardStep(0)}><I d={ic.back} /></button><span style={s.hdrT}>About You</span><span style={{ fontSize: 12, color: t.mut }}>1 of 2</span></div>
      <div style={{ ...s.sec, ...formW }}>
        <div style={{ marginBottom: 14 }}><label style={{ fontSize: 13, fontWeight: 600, display: "block", marginBottom: 4 }}>What should we call you? *</label><input style={s.inp} placeholder="e.g. Sarah" value={f.name} onChange={e => setF(p => ({ ...p, name: e.target.value }))} /></div>
        <div style={{ marginBottom: 14 }}><label style={{ fontSize: 13, fontWeight: 600, display: "block", marginBottom: 4 }}>Your suburb *</label><div style={{ display: "flex", gap: 8 }}><input style={{ ...s.inp, flex: 1 }} placeholder="e.g. Subiaco" value={f.suburb} onChange={e => setF(p => ({ ...p, suburb: e.target.value }))} /><select style={{ ...s.sel, width: 80 }} value={f.state} onChange={e => setF(p => ({ ...p, state: e.target.value }))}>{["WA", "NSW", "VIC", "QLD", "SA", "TAS", "NT", "ACT"].map(x => <option key={x}>{x}</option>)}</select></div></div>
        <div style={{ marginBottom: 14 }}><label style={{ fontSize: 13, fontWeight: 600, display: "block", marginBottom: 4 }}>Quick bio (optional)</label><textarea style={s.ta} placeholder="What do you love baking?" value={f.bio} onChange={e => setF(p => ({ ...p, bio: e.target.value }))} /></div>
    ...
    if (!profile && onboardStep === 2) return <>
      <div style={s.hdr}><button style={s.bck} onClick={() => setOnboardStep(1)}><I d={ic.back} /></button><span style={s.hdrT}>First Item</span><span style={{ fontSize: 12, color: t.mut }}>2 of 2</span></div>
      <div style={{ ...s.sec, ...formW }}>
        <MenuItemForm mf={mf} setMf={setMf} s={s} />
        <div style={{ margin: "16px 0", padding: 16, background: t.card, borderRadius: t.r, boxShadow: t.sh }}>
          <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 8 }}>Before you go live</div>
          <div style={{ fontSize: 13, color: t.mut, lineHeight: 1.7, marginBottom: 12 }}>Under Australian food law, home sellers need to notify their local council. For low-risk items this is usually a free or low-cost notification form.</div>
          ...
          <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${t.bdr}` }}><label style={{ display: "flex", gap: 10, cursor: "pointer", fontSize: 13, lineHeight: 1.5 }}><input type="checkbox" checked={agreed} onChange={e => setAgreed(e.target.checked)} style={{ marginTop: 2, flexShrink: 0 }} /><span>I understand I need to notify my local council and will only list shelf-stable items</span></label></div>
        </div>
        <button style={{ ...s.btn(true), opacity: (mf.name && mf.price && agreed && !saving) ? 1 : .5 }} disabled={!mf.name || !mf.price || !agreed || saving} onClick={async () => {
          setSaving(true);
          const ok = await saveProfile({ ...f, avatar_emoji: mf.emoji });
          if (ok) await addMenuItem(mf);
          setSaving(false);
          setOnboardStep(0);
          showToast("You're live! 🎉");
        }}>{saving ? "Saving..." : "Go Live!"}</button>
      </div>
    </>;

    if (addingItem) return <>
      <div style={s.hdr}><button style={s.bck} onClick={() => setAddingItem(false)}><I d={ic.back} /></button><span style={s.hdrT}>Add Item</span></div>
      <div style={{ ...s.sec, ...formW }}>
        <MenuItemForm mf={mf} setMf={setMf} s={s} />
        <button style={{ ...s.btn(true), marginTop: 8, opacity: (mf.name && mf.price && !saving) ? 1 : .5 }} disabled={!mf.name || !mf.price || saving} onClick={async () => {
          setSaving(true);
          await addMenuItem(mf);
          setMf({ name: "", cat: "Cakes", price: "", desc: "", allergens: [], emoji: "🍰" });
          setAddingItem(false); setSaving(false);
          showToast("Item added!");
          loadMyMenu().then(setMyMenu);
        }}>{saving ? "Saving..." : "Add to Menu"}</button>
      </div>
    </>;

    // Dashboard
    return <>
      <div style={s.hdr}><span style={s.hdrT}>Your Kitchen</span></div>
      <div style={{ ...s.sec, ...formW }}>
        <div style={{ ...s.card, padding: 16, marginBottom: 12 }}>
          <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
            <div style={{ width: 52, height: 52, borderRadius: 14, background: t.priL, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26 }}>{profile.avatar_emoji || "🍰"}</div>
        ...
        {!profile.verified && <div style={{ ...s.tip, background: "#fefce8", color: "#854d0e", marginBottom: 12 }}><strong>Reminder:</strong> Notify your local council to get the ✓ badge.<div style={{ marginTop: 8 }}><button onClick={async () => { await supabase.from("profiles").update({ verified: true }).eq("id", session.user.id); loadProfile(session.user.id); }} style={{ ...s.btnS(true), fontSize: 11 }}>I've notified my council ✓</button></div></div>}
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", margin: "16px 0 10px" }}><span style={{ fontWeight: 700, fontSize: 15 }}>Your Menu ({myMenu.length})</span><button style={s.btnS(true)} onClick={() => { setMf({ name: "", cat: "Cakes", price: "", desc: "", allergens: [], emoji: "🍰" }); setAddingItem(true); }}>+ Add Item</button></div>
        ...
        {myMenu.length === 0 && <div style={{ textAlign: "center", padding: 30, color: t.mut }}><div style={{ fontSize: 32, marginBottom: 8 }}>🍰</div>No menu items yet</div>}
      </div>
    </>;
  };
  ...
  // ─── Account ──────────────────────────────────────────────────────────────
  const Account = () => <>
    <div style={s.hdr}><span style={s.hdrT}>Account</span></div>
    <div style={{ ...s.sec, maxWidth: 500, margin: "0 auto" }}>
      <div style={{ ...s.card, padding: 20 }}>
        <div style={{ display: "flex", gap: 14, alignItems: "center", marginBottom: 16 }}>
          <div style={{ width: 56, height: 56, borderRadius: 16, background: t.priL, display: "flex", alignItems: "center", justifyContent: "center" }}><I d={ic.user} s={24} c={t.pri} /></div>
          <div><div style={{ fontWeight: 700, fontSize: 17 }}>{profile?.name || "User"}</div><div style={{ fontSize: 13, color: t.mut }}>{session.user.email}</div></div>
        </div>
        <div style={{ padding: 12, background: t.bg, borderRadius: t.rs, marginBottom: 16 }}><div style={{ fontSize: 12, color: t.mut, marginBottom: 4 }}>Version</div><div style={{ fontWeight: 600, fontSize: 14 }}>HomeBaked v0.2.0 (Supabase)</div></div>
  ...
  const MenuItemForm = ({ mf, setMf, s: _s }) => <>
    <div style={{ marginBottom: 12 }}><label style={{ fontSize: 13, fontWeight: 600, display: "block", marginBottom: 4 }}>What are you selling? *</label><input style={_s.inp} placeholder="e.g. Lemon Drizzle Cake" value={mf.name} onChange={e => setMf(p => ({ ...p, name: e.target.value }))} /></div>
    ...
    <div style={{ marginBottom: 12 }}><label style={{ fontSize: 13, fontWeight: 600, display: "block", marginBottom: 4 }}>Description (optional)</label><input style={_s.inp} placeholder="e.g. Rich & fudgy" value={mf.desc} onChange={e => setMf(p => ({ ...p, desc: e.target.value }))} /></div>
  ...
  return <div style={s.page}>
    ...
    {bp.mobile && <nav style={s.nav}>{navItems.map(x => <button key={x.id} style={s.navB(tab === x.id && !view)} onClick={() => { setTab(x.id); setView(null); }}><I d={x.icon} s={22} /><span>{x.label}</span></button>)}</nav>}
  </div>;
}